...
class matrix_polynomial
{
   ...
   public:
...
#endif
        {
            std::unique_ptr<matrix<n-m, m>> buf = std::make_unique<matrix<n-m, m>>();
    ...
    void gen_tau(matrix<n-m, n> &tau, unsigned *per)
    {
        ...
        std::unique_ptr<matrix<n-m, n>> t = std::make_unique<matrix<n-m, n>>();
...
#endif
            std::unique_ptr<matrix<m, n>[]> buf = std::make_unique<matrix<m, n>[]>(max_threads);
