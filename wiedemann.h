...
class BW
{
   ...
   void block_wiedemann(t_sol &sol, Mac &M, orig_sys &sys)
   {
      ...
      std::unique_ptr<z_sp_t> z_sp = std::make_unique<z_sp_t>();
      z_sp->rand();

      typedef matrix_array<m, n, num_iter> ai_t;

      std::unique_ptr<ai_t> ai = std::make_unique<ai_t>();
...
//#endif
      ...
      std::unique_ptr<min_poly_t> min_poly = std::make_unique<min_poly_t>(*ai);
